 ...
 *   Blog.updateTocBtn()  — toggle the mobile FAB by view + viewport
 *   Blog.buildToc()      — slugify h2/h3, render desktop + mobile TOC, scroll-spy
 *   Blog.clearToc()      — tear down when leaving the reader view
 *
 * Cross-file deps (on window.Blog): currentLang, bindRipples.
 * ============================================================ */

(function (Blog) {
  'use strict';

  /* ─── Mobile TOC drawer + FAB (immediate setup at load) ─── */
  const mobileToc = document.getElementById('mobile-toc');
  const tocFab = document.getElementById('toc-fab');
  const mobileTocClose = document.getElementById('mobile-toc-close');
  if (mobileTocClose) mobileTocClose.addEventListener('click', () => mobileToc.classList.remove('open'));
  if (mobileToc) mobileToc.addEventListener('click', (e) => { if (e.target === mobileToc) mobileToc.classList.remove('open'); });
  if (tocFab) {
  ...
  Blog.updateTocBtn = function updateTocBtn() {
    const inReader = document.getElementById('view-reader').style.display !== 'none';
    const isNarrow = window.innerWidth < 1360;
    if (tocFab) tocFab.classList.toggle('visible', inReader && isNarrow);
  };
  window.addEventListener('resize', () => Blog.updateTocBtn());

  /* ─── Desktop TOC sidebar ─── */
  let _tocObserver = null;

  // True if `id` is already used by some OTHER element — keeps generated heading
  // ids from clashing with app ids like #toc-sidebar.
  function idTaken(id, self) {
    const el = document.getElementById(id);
    return !!el && el !== self;
  }

  Blog.buildToc = function buildToc() {
    ...
    // Slugify headings into stable ids; guard against collisions with earlier
    // headings AND ids already present in the page (e.g. <nav id="toc-sidebar">).
    headings.forEach(h => {
    ...
    const label = Blog.currentLang === 'zh' ? '目录' : 'Contents';
  ...
  Blog.clearToc = function clearToc() {
    const sidebar = document.getElementById('toc-sidebar');
