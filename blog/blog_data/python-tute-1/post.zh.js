...
<pre><code>name = input("请输入你的名字：")
print("你好，" + name)</code></pre>
<pre><code class="lang-text">用户输入：Alice
你好，Alice</code></pre>
...
<pre><code>print(2 + 3 * 4)        # 乘法先算
print((2 + 3) * 4)      # 括号先算
print(2 ** 3 + 1)       # 幂先算</code></pre>
...
<pre><code>x = 5.345e5      # 等于 5.345 × 10^5
y = 2.14e2       # 等于 214.0
print(x + y)</code></pre>
<pre><code class="lang-text">534714.0</code></pre>
...
print("年龄：" + str(age))  # 正确</code></pre>
<pre><code class="lang-text">年龄：20</code></pre>

<h3>3.4 float 转 int 会截断小数</h3>
<pre><code>x = float(3.9)
print(int(x))     # 不是四舍五入，是直接丢掉小数部分</code></pre>
...
<pre><code>a = float(input("第一个数："))
b = float(input("第二个数："))
print("平均值：" + str((a + b) / 2))</code></pre>
<pre><code class="lang-text">用户输入：10
用户输入：20
平均值：15.0</code></pre>
...
<pre><code>print(17 // 5)    # 17 ÷ 5 = 3 余 2，取整数部分
print(100 // 7)
print(7 // 7)</code></pre>
...
<pre><code>print(17 % 5)     # 17 = 3×5 + 2，余数是 2
print(100 % 7)
print(10 % 2)     # 整除时余数为 0</code></pre>
...
print(str(years) + "年零" + str(remaining_days) + "天")</code></pre>
<pre><code class="lang-text">2小时17分钟
1年零35天</code></pre>
...
if n % 2 == 0:
    ...
    print(str(n) + " 是奇数")</code></pre>
<pre><code class="lang-text">24 是偶数</code></pre>
...
print(s[6:0:-2])  # 从索引 6 开始，每隔一个，往前走</code></pre>
...
<pre><code>s = "Python"
print(len(s))              # 字符串长度
print(s[len(s) - 1])      # 最后一个字符（等价于 s[-1]）</code></pre>
...
x = 5
print(f"x 的平方是 {x ** 2}，x 加 1 是 {x + 1}")</code></pre>
<pre><code class="lang-text">我叫 Alice，今年 20 岁。
x 的平方是 25，x 加 1 是 6</code></pre>

<h3>7.2 格式规范：数字精度</h3>
<p>在 <code>{}</code> 里用 <code>:</code> 后接格式规范，控制输出的精度、宽度和对齐：</p>
<pre><code>price = 9.5
print(f"{price:.2f}")   # 保留 2 位小数
print(f"{price:.4f}")   # 保留 4 位小数</code></pre>
...
<pre><code>for i in range(3):
    print(f"file_{i:02d}.txt")   # 宽度 2，不足补零</code></pre>
...
print(len(fruits))    # 列表长度</code></pre>
...
print(f"成绩等级：{grade}")</code></pre>
<pre><code class="lang-text">成绩等级：C</code></pre>
