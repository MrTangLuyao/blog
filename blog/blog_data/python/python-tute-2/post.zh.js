...
s = "Hello"
print("ell" in s)        # 子字符串检查</code></pre>
<pre><code class="lang-text">True
False
True</code></pre>
...
<pre><code>x = 15
print(10 &lt; x &lt; 20)     # 等价于 x &gt; 10 and x &lt; 20</code></pre>
...
print(power(3, 3))    # 指定 exp=3</code></pre>
...
print("再见！")</code></pre>
<pre><code class="lang-text">用户输入：hello → 你输入了：hello
用户输入：q → 再见！</code></pre>
...
for i in range(5, 0, -1):
    print(i, end=" ")    # 5 4 3 2 1（倒序）</code></pre>
...
<pre><code>for i in range(1, 4):
    for j in range(1, 4):
        print(f"{i}×{j}={i*j}", end="  ")
    print()    # 内层结束后换行</code></pre>
<pre><code class="lang-text">1×1=1  1×2=2  1×3=3
2×1=2  2×2=4  2×3=6
...
<pre><code>fruits = {"apple", "banana", "cherry", "apple"}  # 重复的 apple 自动去除
print(fruits)
...
print(A &amp; B)    # 交集：两者都有的
print(A | B)    # 并集：两者合并
print(A - B)    # 差集：A 有但 B 没有</code></pre>
...
print(sorted(words, key=lambda w: w[-1]))  # 按最后一个字母排序</code></pre>
