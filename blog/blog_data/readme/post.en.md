...
| `blog/blog-syntax.js` | C tokenizer; runs over `<pre><code>` after body injection. Opt out with `class="lang-text"` / `"no-highlight"` |
...
7. **C-only highlighting.** Other languages render verbatim. For `md`, the interpreter tags non-`c` fences `lang-text` so they're skipped cleanly. Extend `blog-syntax.js` to add languages.
8. **Date format**: ISO 8601 only, or Luxon returns "Invalid DateTime".
9. **Cloudflare**: a repo-root `_headers` with `/blog/* → no-cache` keeps `manifest.js` / bodies fresh. If a deploy doesn't show, check that (and Rocket Loader, which can disturb script timing).
10. **Reader reveal uses `requestAnimationFrame`, not `IntersectionObserver`** — toggling `view-reader` from `display:none` fires the IO before layout settles (blank screen). Don't switch it back to IO.
...
| `blog/blog-syntax.js` | C 分词器；正文注入后跑过 `<pre><code>`。跳过：`class="lang-text"` / `"no-highlight"` |
...
7. **只高亮 C。** 其它语言原样显示。`md` 里解释器会给非 `c` 围栏打 `lang-text` 干净跳过。要加语言就扩展 `blog-syntax.js`。
8. **日期格式**：仅 ISO 8601，否则 Luxon 返回 "Invalid DateTime"。
9. **Cloudflare**：仓库根的 `_headers`（`/blog/* → no-cache`）保证 `manifest.js` / 正文刷新即新。发布没更新先查它（以及可能干扰脚本时序的 Rocket Loader）。
10. **阅读器显形用 `requestAnimationFrame`，不是 `IntersectionObserver`** —— 把 `view-reader` 从 `display:none` 切回时 IO 在 layout 稳定前触发（白屏）。别换回 IO。
